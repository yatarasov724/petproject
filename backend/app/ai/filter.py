...
TICKER_KEYWORDS: dict[str, list[str]] = {
    ...
    "VTBR": ["банк втб", "втб"],
    "T": ["тинькофф", "т-банк", "тбанк", "т-технологии"],
    "CBOM": ["мкб", "московский кредитный банк"],
    "BSPB": ["банк санкт-петербург"],
    "AFKS": ["афк система"],
    "MBNK": ["мтс-банк", "мтс банк"],   # МТС-Банк — до MTSS, чтобы конфликт-резолюция сработала
    "SVCB": ["совкомбанк"],
    ...
    # ── Телеком / IT ─────────────────────────────────────────────────────────────
    "YDEX": ["яндекс", "yandex"],
    "MTSS": ["мтс"],          # МТС-оператор — ПОСЛЕ MBNK, конфликт-резолюция уберёт при нужде
    "RTKM": ["ростелеком"],
    "VKCO": ["вконтакте", "vk компани"],
    "POSI": ["позитив текнолоджис", "positive technologies"],
    "HEAD": ["хедхантер", "хэдхантер", "headhunter"],
    "OZON": ["озон", "ozon"],   # OZPH переопределит для «Озон Фармацевтика»
    "DIAS": ["диасофт"],
    # ── Потребительский / ритейл ──────────────────────────────────────────────────
    "MGNT": ["магнит"],
    "X5": ["x5", "х5", "пятёрочка", "пятерочка", "перекрёсток", "чижик"],
    "FIXR": ["fix price", "фикс прайс"],
...
_COMMODITY_KEYWORDS: list[tuple[str, list[str]]] = [
    ...
    ("опек",  ["LKOH", "ROSN"]),  # ОПЕК, ОПЕК+ — matched via _OPEC_RU_RE, not _word_match
    ("opec",  ["LKOH", "ROSN"]),
    ("золот", ["PLZL"]),           # золото, золотой, золотых
]

# "опек" must not match "опека/опекой" (care/custody). Use word-boundary regex.
...
def extract_tickers(title: str) -> list[str]:
    ...
    for stem, tickers in _COMMODITY_KEYWORDS:
        if stem == "опек":
            if _OPEC_RU_RE.search(padded):
                found.update(tickers)
        elif _word_match(stem, padded):
            found.update(tickers)
